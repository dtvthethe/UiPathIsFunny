...
namespace UiPathIsFunny.Utility
{
    public static class Export
    {
        ...
        public static void ToJSONSummary(List<Activity> activityReports, string folderPath)
        {
            string json = JsonConvert.SerializeObject(activityReports);
        ...
        public static void ToCSVSummary(List<Activity> activityReports, string folderPath)
        {
            ...
            {
                // Create file csv
                Type typeOfMyObject = activityReports[0].GetType();
                PropertyInfo[] properties = typeOfMyObject.GetProperties();
                string header = "";
                foreach (PropertyInfo pi in properties)
                {
        ...
        public static void ToExcelSummary(List<Activity> activityReports, string folderPath)
        {
            ...
            activityReports.ForEach(_ =>
            {
                ...
                // Rows:
                var row = ws.Cell(4, 1);
                row.InsertData(activityReports);

                var cl = ws.Range(4, 1, 3 + activityReports.Count, 4);
                cl.Style.Border.RightBorder = XLBorderStyleValues.Thin;
                cl.Style.Border.LeftBorder = XLBorderStyleValues.Thin;
                cl.Style.Border.TopBorder = XLBorderStyleValues.Thin;
                cl.Style.Border.BottomBorder = XLBorderStyleValues.Thin;
